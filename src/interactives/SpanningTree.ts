...
export interface SpanningTreeIteration {
	round: number;
	isSafe: boolean;
}

export class SpanningTree extends ProgramBased<Processor, SpanningTreeIteration> {
	...
	protected override init(): [ReadonlyMap<number, ReadonlySet<number>>, ReadonlyMap<number, Processor>] {
		...
		for (const [from, _] of map) {
			...
			for (const [to, _] of map) {
				if (from !== to && edges.get(to).size < SpanningTree.MaxNeighbors) {
					possibleNeighbors.push(to);
				}
			}
			while (possibleNeighbors.length > 0 && edges.get(from).size < SpanningTree.MaxNeighbors) {
	...
	protected override stepIteration(): SpanningTreeIteration {
		return {
			round: this.round++,
			isSafe: this.isSafe(),
		};
